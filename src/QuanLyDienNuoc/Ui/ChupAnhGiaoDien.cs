...
public static class ChupAnhGiaoDien
{
    ...
    public static int Chay(string thuMucRa)
    {
        ...
        try
        {
            ...
            // Màn nhập khách từ file nhập đúng một khách từ tờ hoá đơn của khách đó, nên tờ mẫu
            // phải mang tên một người chưa có trong sổ — lấy tờ của khách đang có thì ảnh chỉ
            // thấy dải cảnh báo trùng tên.
            var fileToKhachMoi = Path.Combine(thuMucRa, "to-hoa-don-khach-moi.xls");
            XuatHoaDon.Xuat(
                hoaDon,
                new KhachHang { Ten = "Anh Dũng sắt Hà Đông", DiaChi = "12 Nguyễn Trãi, Hà Đông" },
                fileToKhachMoi,
                ngayIn: new DateTime(2026, 8, 3));
            Ghi($"Đã tạo tờ hoá đơn của khách mới: {fileToKhachMoi}");
            loi += ChupForm(
                thuMucRa,
                "20-nhap-khach-tu-file",
                () => new NhapKhachForm(kho.DuLieu.KhachHangs, 2026, fileToKhachMoi));
